...
async function SelectWadFolder() {
    WadFolderPath = ipcRenderer.sendSync("FileSelect", [
        "Select wad folder",
        "Folder",
    ])[0];
    if (WadFolderPath == "") {
    ...
    let arrayOfFiles = [];
    let Files = getAllFiles(WadFolderPath, arrayOfFiles).filter(
        (item) =>
            !/(Combined.json|Separate.json|Missing.json)/g.test(
                item.toLowerCase()
            )
    );
    ...
    if (BinFiles.length == 0) {
        UTIL.CreateAlert(`Bins not found`);
        return 0;
    }
    ...
    for (let i = 0; i < BinFiles.length; i++) {
        ...
        let tempfile = UTIL.Clone(
            fs.readFileSync(BinFiles[i].slice(0, -4) + ".json", "utf-8")
        );
        ...
        for (let PO_ID = 0; PO_ID < ParticleObject.length; PO_ID++) {
            if (ParticleObject[PO_ID].value.name == "VfxSystemDefinitionData") {
                let tempname = ParticleObject[PO_ID].value.items.find(
                    (item) => {
                ...
                let DefData = ParticleObject[PO_ID].value.items.filter(
                    (item) =>
                    ...
                    for (let C = 0; C < Props.length; C++) {
                        if (
                            ...
                            Matches?.forEach((Match) => {
                                if (
                                    !SeparateOutput[
                                        SeparateOutput.length - 1
                                    ].Files.includes(Match.toLowerCase())
                                ) {
                                    SeparateOutput[
                                        SeparateOutput.length - 1
                                    ].Files.push(Match.toLowerCase());
                                }
                            });
                        }
                    }
    ...
    let AssetFiles = Files.filter((item) =>
        /(particles|shared)/g.test(item.toLowerCase())
    ).filter((item) =>
        /\.(?:dds|skn|skl|sco|scb|anm)/gi.test(item.toLowerCase())
    );
    ...
    for (let i = 0; i < CombinedOutput.length; i++) {
        ...
        for (let j = 0; j < AssetFiles.length; j++) {
            if (
                CombinedOutput[i].toLowerCase().replace(/\//g, "\\") ==
                AssetFiles[j].slice(WadFolderPath.length + 1)
            ) {
                break;
    ...
    for (let i = 0; i < CombinedOutput.length; i++) {
        if (
            SortedCombinedOutput.some(
                (item) => item.Location == deasset(CombinedOutput[i])
            ) != false
        ) {
            SortedCombinedOutput[
                SortedCombinedOutput.findIndex(
                    (item) => item.Location == deasset(CombinedOutput[i])
                )
            ].Files.push(CombinedOutput[i]);
        } else {
            let temp = CombinedOutput[i].split("/");
            temp.pop();
            temp = temp.join("/");
            SortedCombinedOutput.push({
                Location: temp,
                Files: [CombinedOutput[i]],
            });
        }
    }

    fs.writeFileSync(
        WadFolderPath + "\\Combined.json",
        JSON.stringify(SortedCombinedOutput, null, 2),
        "utf8"
    );

    SeparateOutput.sort((a, b) =>
        a.Particle > b.Particle ? 1 : b.Particle > a.Particle ? -1 : 0
    );
    ...
    fs.writeFileSync(
        WadFolderPath + "\\Separate.json",
        JSON.stringify(SeparateOutput, null, 2),
        "utf8"
    );
...
async function DetectUnused() {
    ...
    let Combined = UTIL.Clone(
        fs.readFileSync(WadFolderPath + "\\Combined.json", "utf-8")
    );
    ...
    for (let i = 0; i < AssetFiles.length; i++) {
        ...
        for (let j = 0; j < Combined.length; j++) {
            location: for (let k = 0; k < Combined[j].Files.length; k++) {
                if (
                    AssetFiles[i]
                        .toLowerCase()
                        .endsWith(Combined[j].Files[k].replace(/\//g, "\\"))
                ) {
                    break location;
                } else if (
                    j ==
                    Combined[Combined.length - 1].Files.length - 1
                ) {
                    UnusedCount++;
                    if (!Files2Delete.includes(AssetFiles[i])) {
    ...
    Progress.classList.add("Progress-Complete");
    UTIL.CreateAlert(
        `${UnusedCount} Total files detected`
    );
}
...
function DeleteUnused() {
    ...
    UTIL.CreateAlert(
        `Deleted ${Files2Delete.length} unused files`
    );
}
