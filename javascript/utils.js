...
class Preferences {
  ...
  RitoBinPath() {
    this.obj.RitoBinPath = ipcRenderer.sendSync("FileSelect", [
      "Select RitoBin_cli.exe",
      "RitoBin",
    ]);
    this.save();
  }
  FFMPEGPath() {
    this.obj.FFMPEGPath = ipcRenderer.sendSync("FileSelect", [
      "Select ffmpeg.exe",
      "ffmpeg",
    ]);
    this.save();
...
class SampleDB {
  constructor(SamplePath) {
    ...
    for (let i = 0; i < this.obj.length; i++) {
      ...
      for (let j = 0; j < element.Palette.length; j++) {
        element.Palette[j] = new ColorHandler(
          element.Palette[j].vec4,
          element.Palette[j].time
        );
      }
    }
  }
  ...
  export(ID = undefined) {
    let Folder = ipcRenderer.sendSync("FileSelect", [
      "Select sample export location",
      "Folder",
    ]);
    if (ID != undefined) {
  ...
  import() {
    let File = ipcRenderer.sendSync("FileSelect", ["Import Samples", "Json"]);
  ...
  Object.defineProperty(String.prototype, "fnv", {
    value: function (expected) {
      return (
        ...
        fnv1a(expected.toLowerCase()) == this
      );
    },
    enumerable: false,
  });
